...
Function ExportSecrets {
    Param(
        ...
        [parameter(Mandatory = $true)][System.IO.FileInfo] $FileName
    );
...
Function ImportSecrets {
    ...
    if (!(Test-Path -Path $FileName)) {
        ...
        if ($Description.Length -gt 0) {
            ...
            $Tags = @{ 'Priority' = 'medium'; 'Department' = 'GoodDepartment'; 'Source' = $FileName; 'Description' = $Description }
        }
        else {
            $Tags = @{ 'Priority' = 'medium'; 'Department' = 'GoodDepartment'; 'Source' = $FileName; 'Description' = '' }    
        }
